...
class PressureMapMaker:
    def __init__(self, 
        ...
        self.circle_template = np.zeros([2*(image_size[0] + contact_radius_pix) + 1, 2*(image_size[1] + contact_radius_pix) + 1])
        ...
        for i in range(self.circle_template.shape[0]):
            for j in range(self.circle_template.shape[1]):
                ...
                if dist < contact_radius_pix:
                    self.circle_template[i,j] = 1.0
                else:
                    self.circle_template[i,j] = 0.0

        self.template_center = center
        
    def __call__(self, force:float, contact_pos:Tuple[float,float]) -> np.ndarray:
        # calculate the average pressure:
        avg_pressure = force*self.force_mult / (np.pi * self.contact_radius**2)
        ...
        # multiply the template by the average pressure
        pressure_map = adj_template * avg_pressure * self.out_mult
